...
public class CommandGMS implements TabExecutor {
    private final Ecotropolis plugin;
    public CommandGMS(Ecotropolis plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String Label, String[] args) {
        ...
        if (args.length == 1) {
            ...
            p.sendMessage(Util.transform(plugin.prefix + "&3 " + target.getName() + "'s gamemode changed to &6Survival Mode!"));
            return true;
        }
        ...
        p.sendMessage(Util.transform(plugin.prefix + "&3Gamemode changed to &6Survival Mode"));
        return true;
    }
